...
let controlPts = [];	// array for control points
let curvePts = [];		// array for curve points
...
//#begin
	...
	for(let i=0;i<(closedCurve?npts:(npts-1));i++) {
		...
		A.x = 2*p[i].x-2*p[(i+1)%npts].x+p[i].dx*2+p[(i+1)%npts].dx*2;
		A.y = 2*p[i].y-2*p[(i+1)%npts].y+p[i].dy*2+p[(i+1)%npts].dy*2;
		
		B.x =-3*p[i].x+3*p[(i+1)%npts].x-2*p[i].dx*2-p[(i+1)%npts].dx*2;
		B.y =-3*p[i].y+3*p[(i+1)%npts].y-2*p[i].dy*2-p[(i+1)%npts].dy*2;
...
function loadPreset(idx) {
	let ptsname = 'pts'+idx;
	if(eval('typeof('+ptsname+')')=='undefined') {
		message(ptsname+' does not exist');
		return;
	}
	let norm_xy = eval(ptsname);
...
let controlParams = function() {
  ...
  this['Save Control Points'] = function() {
	  ...
	  let text = 'let pts='+JSON.stringify(norm_xy)+';';
	  let blob = new Blob([text], {type: 'text/plain;charset=utf-8'});
	  saveAs(blob, 'pts.js');
  };
};
...
/*
function render() {
	if(renderer) renderSurfaceViewer();
}
